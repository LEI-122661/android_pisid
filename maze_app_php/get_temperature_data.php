...
$sql = "SELECT IDTemperatura, IDMensagem, Hora, Temperatura FROM Temperatura ORDER BY IDTemperatura DESC LIMIT 50";
$result = $conn->query($sql);

if ($result) {
    $data = array();
    while ($row = $result->fetch_assoc()) {
        $row['IDTemperatura'] = (int)$row['IDTemperatura'];
        $row['IDMensagem'] = $row['IDMensagem'] !== null ? (int)$row['IDMensagem'] : null;
        $row['Temperatura'] = (float)$row['Temperatura'];
        $data[] = $row;
    }
    $response['success'] = true;
    $response['data'] = array_reverse($data); // Ordem cronológica para o gráfico
} else {
